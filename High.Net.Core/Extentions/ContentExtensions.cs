...
namespace High.Net.Core.Extentions
{
    public static class ContentExtensions
    {
        ...
        /// <param name="contentReference">Content reference for which to get page.</param>
        /// <returns>Page of <typeparamref name="T" /> that match content reference.</returns>
        public static T TryGetPage<T>(this ContentReference contentReference) where T : PageData
        {
            var content = default(T);
            if (contentReference.IsNullOrEmpty()) return content;

            var loader = ServiceLocator.Current.GetInstance<IContentRepository>();
            loader.TryGet<T>(contentReference, out content);
            return content;
        }
        ...
        public static string GetImageName(this ContentReference contentReference)
        {
            ...
            try
            {
                ...
                {
                    imageName = contentReference.GetFriendlyUrl().Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries).LastOrDefault().Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault();
                }
            }
            catch
            {
        ...
        public static List<T> Items<T>(this ContentArea contentArea)
        {
            return contentArea != null ? contentArea.FilteredItems.Select(i => (T)i.GetContent()).ToList() : new List<T>();
        }
    }
}
