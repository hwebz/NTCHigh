...
namespace High.Net.Core
{
    public static class HtmlHelpers
    {
        ...
        public static MvcHtmlString DisplayHiddenFor<TModel, TValue>(this HtmlHelper<TModel> helper, System.Linq.Expressions.Expression<Func<TModel, TValue>> expression)
        {
            return DisplayHiddenFor(helper, expression, null);
        }

        public static MvcHtmlString DisplayHiddenFor<TModel, TValue>(this HtmlHelper<TModel> helper, System.Linq.Expressions.Expression<Func<TModel, TValue>> expression, object htmlAttributes)
        {
            ...
            hidden.Attributes.Add("id", htmlFieldName.Replace('.', '_'));
            if (metadata.Model != null)
            {
                hidden.Attributes.Add("value", metadata.Model.ToString());
            }

            mainDiv.InnerHtml += hidden;
            mainDiv.InnerHtml += metadata.Model;

            mainDiv.MergeAttributes(new System.Web.Routing.RouteValueDictionary(htmlAttributes));
        ...
        {
            var version = GetVersion(helper, filename);
            return MvcHtmlString.Create("<link rel=\"stylesheet\" type =\"text/css\" href=\""+  filename + version + "\"/>");
        }
        ...
        private static string GetVersion(this HtmlHelper helper, string filename)
        {
            ...
            if (context.Cache[cacheKey] == null)
            {
                ...
                var version = $"?v={new FileInfo(physicalPath).LastWriteTime:MMddHHmmss}";
                context.Cache.Add(cacheKey, version, new CacheDependency(physicalPath, DateTime.Now),
            ...
            {
                return context.Cache[cacheKey] as string;
            }
        }

        private static string BuildCacheKey(string filename)
        {
            return $"ClientResourceVersioning_cache_{filename}";
        }
        ...
        public static string[] ToListLineString(this string original)
        {
            if (string.IsNullOrEmpty(original)) return new string[0];
            return original.Contains("\n") ? original.Split(new string[] { "\n"}, StringSplitOptions.None) : new string[] { original };         
        }

        public static string MakeImageBackground(this HtmlHelper helper, string url)
        {
            return string.IsNullOrEmpty(url) ? string.Empty : $"background-image: url('{url}')";
        }
    }
}
